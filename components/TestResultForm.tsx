...
interface SchoolFormProps {
  addSchool: (school: Omit<School, 'id' | 'dateAdded'>) => void;
  addMultipleSchools: (schools: Omit<School, 'id' | 'dateAdded'>[]) => void;
...
export const SchoolForm: React.FC<SchoolFormProps> = ({ addSchool, updateSchool, editingSchool, cancelEdit, addMultipleSchools }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      ...
      addSchool(formData as Omit<School, 'id' | 'dateAdded'>);
      setFormData(initialFormData);
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md sticky top-8">
      <h2 className="text-2xl font-bold mb-4 text-slate-700">{isEditing ? 'تعديل بيانات المدرسة' : 'إضافة مدرسة جديدة'}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-h-[75vh] overflow-y-auto pr-2">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            {renderInput('nationalId', 'الرقم الوطني*', { placeholder: '200100...' })}
        </div>
        ...
        <div className="flex items-center gap-4 mt-4">
          ...
          >
            {isEditing ? 'حفظ التعديلات' : 'إضافة المدرسة'}
          </button>
          {isEditing && (
            <button
              type="button"
