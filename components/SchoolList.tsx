...
export const SchoolList: React.FC<SchoolListProps> = ({ schools, removeSchool, editSchool, isManager = false }) => {
  const [searchTerm, setSearchTerm] = useState('');

  const handleExport = () => {
    if (typeof XLSX === 'undefined') {
        alert('مكتبة التصدير غير متاحة. يرجى المحاولة مرة أخرى.');
        return;
    }

    const headers = {
        schoolNameAr: 'اسم المدرسة (العربية)',
        schoolNameEn: 'اسم المدرسة (الإنجليزية)',
        nationalId: 'الرقم الوطني',
        schoolId: 'الرقم الخاص للمدرسة',
        region: 'المنطقة',
        principalName: 'اسم المدير',
        principalEmail: 'ايميل المدير',
        principalPhone: 'هاتف المدير',
        lowestGrade: 'أدنى صف',
        highestGrade: 'أعلى صف',
        schoolGender: 'جنس المدرسة',
        buildingType: 'نوع المبنى',
        isCamp: 'مدرسة مخيم؟',
        dateAdded: 'تاريخ الإضافة',
    };

    const dataForSheet = schools.map(school => ({
        [headers.schoolNameAr]: school.schoolNameAr,
        [headers.schoolNameEn]: school.schoolNameEn,
        [headers.nationalId]: school.nationalId,
        [headers.schoolId]: school.schoolId,
        [headers.region]: school.region,
        [headers.principalName]: school.principalName,
        [headers.principalEmail]: school.principalEmail,
        [headers.principalPhone]: school.principalPhone,
        [headers.lowestGrade]: school.lowestGrade,
        [headers.highestGrade]: school.highestGrade,
        [headers.schoolGender]: school.schoolGender,
        [headers.buildingType]: school.buildingType,
        [headers.isCamp]: school.isCamp ? 'نعم' : 'لا',
        [headers.dateAdded]: new Date(school.dateAdded).toLocaleDateString('ar-EG'),
    }));
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'قائمة المدارس');
    XLSX.writeFile(wb, `قائمة_المدارس_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
    ...
    <div className="bg-white rounded-xl shadow-md">
      <div className="p-6 border-b border-slate-200">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
            <h2 className="text-2xl font-bold text-slate-700">قائمة المدارس ({schools.length})</h2>
            {schools.length > 0 && !isManager && (
                <button
                    onClick={handleExport}
                    className="flex items-center justify-center gap-2 bg-green-600 text-white font-bold py-2 px-4 rounded-md hover:bg-green-700 transition w-full sm:w-auto shrink-0"
                    aria-label="تصدير قائمة المدارس إلى ملف Excel"
                >
